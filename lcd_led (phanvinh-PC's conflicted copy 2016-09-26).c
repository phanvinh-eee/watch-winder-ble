...
extern Motor_Typedef Mo[24];
extern uint8_t mNum;
extern MCH_Typedef MCH_Set[4];
...
void control(void){
  ...
  else if(GPIO_ReadInputDataBit(GPIOB , GPIO_Pin_4)==0)
  {
    ...
    Mo[mLine].Di=MCH_Set[mMCH].Di;
		if(mLine>5){
			sprintf(str,"mch%02d%d%d%d%d",mLine-6,mMCH,mTPD,mDir,SLEEP);
  ...
  else if(GPIO_ReadInputDataBit(GPIOB , GPIO_Pin_3)==0)
  {
    ...
    else
    {
      ...
      Mo[mLine].Di= MCH_Set[mMCH].Di;            
		...
		if(mLine>5){
			sprintf(str,"poo%02d%d%d%d%d",mLine-6,mMCH,mTPD,mDir,Mo[mLine].Pow);
			USART_SendString(USART2,str);
		}
  }
}
